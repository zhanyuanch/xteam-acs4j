...
import com.xteam.tools.config4j.cfg.XmlUtils;
...
public class XmlBuilder {
	...
	public static void main(String[] args) throws Exception{
		XmlBuilder cb	= new XmlBuilder("config/db-config.xml");
		 ...
		 * 
		 * ROOT.NODE.CNODE:$children   末结点的所有孩子列表 	返回 List<XmlNode>
		...
		try {
			...
			//--获取 ccc 结点自己 
			XmlNode obj1	= (XmlNode)cb.getRoot().getValueByPath("servers.dbserver.ccc:$node");
			...
			//--获取dbserver 结点的 孩子结点表 List<XmlNode>
	 ...
	 * @param xmlTxt xml 文本字符串
	 * @param encoding xml 文本字符串的编码方式，为空是采用默认编码
	 */
	public XmlBuilder(String xmlTxt,String encoding){
		if(root==null) this.root	= new XmlNode();
	 ...
	 * @param e
	 */
	private void readConfig(XmlNode parent,Element e){
