...
public class ConfigUtils {
	 ...
	 * @param cnd
	 */
	public static Element nodeAdd2Element(Element src,ConfigNode node){
		Element elm	= src;
		//====设置结点名和值========
		...
		//====设置子结点================
		List<ConfigNode> clist	= node.getChildrenList();
		if(clist!=null && clist.size()>0){
			Iterator<ConfigNode> it	= clist.iterator();
			while(it.hasNext()){
	 ...
	 * @return
	 */
	public static Object getValueByPath(ConfigNode configNode,String path){
		Object rtn	= null;
		...
		ConfigNode last	= getLastNode(configNode, path);	//获取路径中最后一个结点
	 ...
	 * @return
	 */
	public static Object getValueByKey(ConfigNode configNode,String key){
		Object rtn	= null;
	 ...
	 * @return
	 */
	public static ConfigNode getLastNode(ConfigNode configNode,String path){
		ConfigNode rtn	= configNode;
		String[] ps	= parseValueStr(path);
		if(ps!=null){
			for(int i=1;i<ps.length;i++){
				if(rtn!=null){
					String[] cps	= ConfigUtils.parseNodeStr(ps[i]);
					if(cps!=null){
					//----获取指定条件的儿子结点
						rtn	= ConfigUtils.getOneNodeByAttribute(rtn,cps[0], cps[1], cps[2]);
					}else{
					//----没有条件获取默认儿子结点
						rtn	= ConfigUtils.getOneChildNode(rtn,ps[i]);
					}
				}
			}
		}
	 ...
	 * @return
	 */
	public static ConfigNode getOneChildNode(ConfigNode cfgnode,String name){
		ConfigNode node	= null;
		for(ConfigNode cnode :cfgnode.getChildrenList()){
	 ...
	 * @return
	 */
	public static ConfigNode getOneNodeByAttribute(ConfigNode configNode,String nodename,String attname,String attvalue){
		List<ConfigNode> nlist	= configNode.getChildrenByName(nodename);
		ConfigNode rtn	= null;
		if(nlist!=null && nlist.size()>0){
			for(ConfigNode cnode:nlist){
